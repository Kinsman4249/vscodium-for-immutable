...
- `.gitignore` now ignores the whole `.kilo/` directory (`.kilo/`) rather than just `.kilo/kilo.jsonc`, so Kilo's local tool state - including installed `node_modules` - cannot be swept into a release by `git add -A` at the release-gh G9 step.

### Fixed

- The ASCII-lint mismatch the Claude-Skills-and-.md-files. backup repo reports from its release gate is closed. GNU grep 3.8 rejects `\x{...}` escapes above the BMP (`\x{1FAFF}`) unless it runs in a UTF-8 locale, and the agent's Bash tool does not always inherit the interactive shell's locale - previously the gate errored and was worked around with a perl equivalent. The backup repo's G7 gate now forces `LC_ALL=C.UTF-8` (shipped there in v1.16.0), and this box's launchers (`compute_env_args` in `install-vscodium.sh`) now add `LANG=C.UTF-8` and `LC_ALL=C.UTF-8` to every session they start, so the failure cannot reappear even for processes that never source `/etc/bash.bashrc`.
