...
- The ASCII-lint mismatch the Claude-Skills-and-.md-files. backup repo reports from its release gate is the container's grep build: GNU grep 3.8 here rejects `\x{...}` escapes for code points above the BMP (`\x{1FAFF}` -> "character code point value ... is too large"), while the same pattern is accepted on GitHub Actions' ubuntu-latest. The gate is equivalent-checked with perl's codepoint class on this machine until the pattern changes.
